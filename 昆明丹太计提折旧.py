# -*- coding: utf-8 -*-
"""
Created on Thu Aug 30 13:45:02 2018
...
os.getcwd()
excel_path = 'E:\\OneDrive\\跌价\\昆明丹太库存.xlsx'  #设定读取excel路径
...
def SQL(sql, zt):
    ...
    import pandas as pd
    conn = pyodbc.connect(
        r'DRIVER=SQL SERVER;SERVER=183.64.79.10;UID=bb;PWD=cqhg65730332')
    cursor = conn.cursor()
    cursor.execute(zt)
...
restult = pd.DataFrame(SQL(sql, "use saledata010"))
...
result2 = pd.DataFrame(SQL(sql2, "use saledata010"))  #读取现在的库存
c = result2.loc[(result2['入库时间'] > '2016-1-25') & (result2['部门名称'] == "丹太配件组"),
                ['件号', '数量', '关联标志', '入库时间', '单价', '合计金额']]  #筛选早于2016-1-1的库存
c = c.reset_index()
nf = ('2017-08-25', '2016-08-25', '2015-08-25', '2014-08-25')
...
a['年限'] = a['入库时间'].map(nx)
df1 = c.loc[:, ['件号', '数量', '单价', '年限']]  #按照列进行选择
df2 = a.loc[:, ['件号', '数量', '单价', '年限']]
df3 = [df1, df2]
df = pd.concat(df3)  #两个表格合并
df['合计金额'] = df['数量'] * df['单价'].astype('int')
y = df.groupby('年限')['合计金额'].sum()
y = y.reset_index()
